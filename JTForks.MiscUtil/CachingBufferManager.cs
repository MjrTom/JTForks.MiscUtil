// <copyright file="CachingBufferManager.cs" company="MjrTom">
...
namespace MiscUtil
{
    ...
    /// buffers. The precise behavior is controlled by the nested Options
    /// class.
    ///
    ...
    public sealed class CachingBufferManager : IBufferManager
    {
        ...
        /// </summary>
        private readonly List<CachedBuffer?[]?> bufferBands = [];
        /// <summary>
        /// Lock for member access. 5 seconds should be more than adequate
        /// as a timeout.
        /// </summary>
        private readonly ThreadingSyncLock padlock = new("Lock for CachingBufferManager", 5000);
        ...
        public IBuffer GetBuffer(int minimumSize)
        {
            ...
            if (minimumSize > this.options.MaxBufferSize)
            {
                throw new BufferAcquisitionException("Requested buffer " + minimumSize + " is larger " +
                                                     "than maximum buffer size " + this.options.MaxBufferSize);
            }

            // Work out the size of buffer to use, and where in the list to find
            // cached buffers
            var listIndex = 0;
            var size = this.options.MinBufferSize;
            while (size < minimumSize)
            {
                size = this.CalculateNextSizeBand(size);
                listIndex++;
            }
            ...
            while (true)
            {
                CachedBuffer? ret = this.FindAvailableBuffer(listIndex, size);
                if (ret != null)
                {
                    return ret;
                }

                switch (this.options.ActionOnBufferUnavailable)
                {
                    case Options.BufferUnavailableAction.ReturnUncached:
                        return new CachedBuffer(minimumSize, this.options.ClearAfterUse);
        ...
        {
            return (int)Math.Ceiling(Math.Min(this.options.MaxBufferSize,
                                              size * this.options.ScalingFactor));
        }
        ...
        private CachedBuffer? FindAvailableBuffer(int listIndex, int size)
        {
            using (this.padlock.Lock())
            {
                ...
                while (listIndex >= this.bufferBands.Count)
                {
                    this.bufferBands.Add(null);
                }

                // Create a new array of buffers if necessary
                CachedBuffer?[]? buffers = this.bufferBands[listIndex];
                if (buffers == null)
                {
                    buffers = new CachedBuffer[this.options.MaxBuffersPerSizeBand];
                    this.bufferBands[listIndex] = buffers;
                }

                // Look through all the buffers in this band for an available one, or an unused slot
                for (var i = 0; i < buffers.Length; i++)
                {
                    CachedBuffer? buffer = buffers[i];
                    // No cached unused buffers. Create a new one.
                    if (buffer == null)
                    {
                        buffer = new CachedBuffer(size, this.options.ClearAfterUse);
                        buffers[i] = buffer;
                        return buffer;
                    }
...
= 16;
            ...
            public int MinBufferSize
            {
                ...
                set
                {
                    ArgumentOutOfRangeException.ThrowIfLessThan(value, 1, "Minimum buffer size must be at least 1");
                    field = value;
                }
            }
= 1024;
            ...
            public double ScalingFactor
            {
                ...
                set
                {
                    ArgumentOutOfRangeException.ThrowIfLessThan(value, 1.25, "Scaling factor must be at least 1.25.");
                    field = value;
                }
            }
= 2.0;
...
= int.MaxValue;
            ...
            public BufferUnavailableAction ActionOnBufferUnavailable
            {
                ...
                set
                {
                    field = !Enum.IsDefined<BufferUnavailableAction>(value)
                    ? throw new ArgumentOutOfRangeException(nameof(value), "Only defined values are permitted")
                    : value;
                }
            }
= BufferUnavailableAction.ReturnUncached;
