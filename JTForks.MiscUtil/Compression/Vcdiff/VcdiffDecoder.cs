// <copyright file="VcdiffDecoder.cs" company="MjrTom">
...
namespace MiscUtil.Compression.Vcdiff
{
    ...
    public sealed class VcdiffDecoder : IDisposable
    {
        ...
        public static void Decode(Stream original, Stream delta, Stream output)
        {
            ...
            if (!delta.CanRead)
            {
                throw new ArgumentException("Unable to read from delta stream", nameof(delta));
            }
        ...
        /// <summary>
        /// Disposes of the decoder and its underlying resources.
        /// </summary>
        public void Dispose()
        {
            this.cache.Dispose();
        ...
        /// </summary>
        /// <exception cref="VcdiffFormatException"></exception>
        private void ReadHeader()
        {
            var header = IOHelper.CheckedReadBytes(this.delta, 4);
            if (header[0] != 0xd6 ||
                header[1] != 0xc3 ||
                header[2] != 0xc4)
            {
            ...
            var headerIndicator = IOHelper.CheckedReadByte(this.delta);
            ...
            {
                var appHeaderLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
                IOHelper.CheckedReadBytes(this.delta, appHeaderLength);
            }
        }
        ...
        private void ReadCodeTable()
        {
            ...
            var compressedTableLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta) - 2;
            int nearSize = IOHelper.CheckedReadByte(this.delta);
            int sameSize = IOHelper.CheckedReadByte(this.delta);
            var compressedTableData = IOHelper.CheckedReadBytes(this.delta, compressedTableLength);
            ...
            this.codeTable = new CodeTable(decompressedTableData);
            this.cache = new AddressCache(nearSize, sameSize);
        }
        ...
        {
            var windowIndicator = this.delta.ReadByte();
            // Have we finished?
            ...
            var sourceStreamPostReadSeek = -1;
            ...
            var hasAdler32Checksum = (windowIndicator & 4) == 4;
            ...
            switch (windowIndicator & 3)
            {
                ...
                case 1:
                    ...
                    sourceStream = this.original;
                    break;
                case 2:
                    sourceStream = this.output;
                    sourceStreamPostReadSeek = (int)this.output.Position;
                    break;
                case 3:
                    throw new VcdiffFormatException
            ...
            {
                sourceLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
                var sourcePosition = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
            ...
            IOHelper.ReadBigEndian7BitEncodedInt(this.delta);

            // Read how long the target window is
            var targetLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
            ...
            var deltaIndicator = IOHelper.CheckedReadByte(this.delta);
            if (deltaIndicator != 0)
            {
                throw new VcdiffFormatException("VcdiffDecoder is unable to handle compressed delta sections.");
            }

            var addRunDataLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
            var instructionsLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
            var addressesLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);

            // If we've been given a checksum, we have to read it and we might as well
            // use it to check the data.
            var checksumInFile = 0;
            if (hasAdler32Checksum)
            {
                var checksumBytes = IOHelper.CheckedReadBytes(this.delta, 4);
            ...
            var addRunData = IOHelper.CheckedReadBytes(this.delta, addRunDataLength);
            var instructions = IOHelper.CheckedReadBytes(this.delta, instructionsLength);
            var addresses = IOHelper.CheckedReadBytes(this.delta, addressesLength);

            var addRunDataIndex = 0;

            MemoryStream instructionStream = new(instructions, false);

            this.cache.Reset(addresses);
            ...
            while (true)
            {
                ...
                for (var i = 0; i < 2; i++)
                {
                    Instruction instruction = this.codeTable[instructionIndex, i];
                    ...
                    switch (instruction.Type)
                    {
                        ...
                        case InstructionType.Copy:
                            var addr = this.cache.DecodeAddress((int)targetDataStream.Position + sourceLength, instruction.Mode);
            ...
            this.output.Write(targetData, 0, targetLength);

            if (hasAdler32Checksum)
            {
                var actualChecksum = Adler32.ComputeChecksum(1, targetData);
