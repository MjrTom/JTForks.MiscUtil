// <copyright file="Utf32String.cs" company="MjrTom">
...
namespace MiscUtil.Text
{
    ...
    /// </summary>
    public sealed class Utf32String : IEnumerable<int>, IComparable, ICloneable
    {
        ...
        public int this[int index] => this.characters[index];
        ...
        {
            ArgumentNullException.ThrowIfNull(utf16);
            // Assume no surrogates to start with
            this.characters = new int[utf16.Length];
            var highSurrogate = -1;
            var outputIndex = 0;
            for (var i = 0; i < utf16.Length; i++)
            {
                var c = utf16[i];
                if (c is >= (char)HighSurrogateStart and <= (char)HighSurrogateEnd)
                {
                    if (highSurrogate != -1)
                    {
                        throw new ArgumentException("Invalid string: two high surrogates in a row", nameof(utf16));
                    }

                    highSurrogate = (c - HighSurrogateStart) * 0x400;
                    continue;
                }
                else if (c is >= (char)LowSurrogateStart and <= (char)LowSurrogateEnd)
                {
                    if (highSurrogate == -1)
                    {
                    ...
                    this.characters[outputIndex++] = highSurrogate + (c - LowSurrogateStart) + 0x10000;
                    highSurrogate = -1;
                }
                else
                {
                    if (highSurrogate != -1)
                    {
                        throw new ArgumentException("Invalid string: high surrogates with no following low surrogate", nameof(utf16));
                    }

                    this.characters[outputIndex++] = c;
                }
            }
            ...
            if (outputIndex != this.characters.Length)
            {
                var tmp = new int[outputIndex];
                Array.Copy(this.characters, 0, tmp, 0, outputIndex);
                this.characters = tmp;
            }
        }
        ...
        public Utf32String Substring(int start)
        {
            ...
            ArgumentOutOfRangeException.ThrowIfGreaterThan(start, this.Length);

            return start == this.Length ? Utf32String.Empty : this.Substring(start, this.Length - start);
        }
        ...
        public Utf32String Substring(int start, int count)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(count), "start+count must be less than or equal to the length of the string");
            }
            ...
            Array.Copy(this.characters, start, tmp, 0, count);
        ...
        {
            return this.IndexOf(character, 0, this.Length);
        }
        ...
        public int IndexOf(int character, int start)
        {
            ArgumentOutOfRangeException.ThrowIfGreaterThan(start, this.Length);
            ArgumentOutOfRangeException.ThrowIfNegative(start);
            return this.IndexOf(character, start, this.Length - start);
        }
        ...
        {
            return Object.ReferenceEquals(this, other) || this.CompareTo(other) == 0;
        }
        ...
        {
            return ReferenceEquals(strA, strB) ? 0 : strA is null ? -1 : strB is null ? 1 : strA.CompareTo(strB);
        }
        ...
        {
            return (int[])this.characters.Clone();
        }
        ...
        public override string ToString()
        {
            ...
            var utf16 = new char[this.Length + surrogates];
            var outputIndex = 0;
            foreach (var character in this.characters)
            {
        ...
        {
            var hash = 0;
            var step = Math.Max(this.Length / HashcodeSampleSize, 1);
            for (var i = 0; i < this.Length; i += step)
            {
                hash ^= this.characters[i];
            }
        ...
        /// <summary>
        /// Compares two strings to see which is lexicographically first.
        /// </summary>
        /// <param name="strA">The first string</param>
        /// <param name="strB">The second string</param>
        /// <returns>true if strA is lexicographically first; otherwise, false</returns>
        public static bool operator <(Utf32String? strA, Utf32String? strB)
        {
            return Compare(strA, strB) < 0;
        }
        ...
        /// <summary>
        /// Compares two strings to see which is lexicographically last.
        /// </summary>
        /// <param name="strA">The first string</param>
        /// <param name="strB">The second string</param>
        /// <returns>true if strA is lexicographically last; otherwise, false</returns>
        public static bool operator >(Utf32String? strA, Utf32String? strB)
        {
            return Compare(strA, strB) > 0;
        }
        ...
        /// <summary>
        /// Compares two strings to see if the first is lexicographically first or equal.
        /// </summary>
        /// <param name="strA">The first string</param>
        /// <param name="strB">The second string</param>
        /// <returns>true if strA is lexicographically first or equal; otherwise, false</returns>
        public static bool operator <=(Utf32String? strA, Utf32String? strB)
        {
            return Compare(strA, strB) <= 0;
        }
        ...
        /// <summary>
        /// Compares two strings to see if the first is lexicographically last or equal.
        /// </summary>
        /// <param name="strA">The first string</param>
        /// <param name="strB">The second string</param>
        /// <returns>true if strA is lexicographically last or equal; otherwise, false</returns>
        public static bool operator >=(Utf32String? strA, Utf32String? strB)
        {
            return Compare(strA, strB) >= 0;
        }
        ...
        {
            return ((IEnumerable<int>)this.characters).GetEnumerator();
        }
        ...
        public int CompareTo(object? obj)
        {
            ...
            Utf32String other = obj as Utf32String ?? throw new ArgumentException("Can only compare Utf32Strings", nameof(obj));
