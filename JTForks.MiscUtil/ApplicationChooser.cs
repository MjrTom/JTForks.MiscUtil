// <copyright file="ApplicationChooser.cs" company="MjrTom">
...
namespace MiscUtil
{
    ...
    public static class ApplicationChooser
    {
        ...
        public static void Run(Type type, string[] args)
        {
            ...
            var entryPoints = new List<MethodBase>();
            ...
            entryPoints.Sort(CompareEntryPoints);
            ...
            var key = (input != null && input.Length > 0) ? char.ToUpperInvariant(input[0]) : '\0';
            ...
            var entry = Keys.IndexOf(char.ToUpperInvariant(key));
        ...
        private static string? GetEntryPointName(MethodBase methodBase)
        {
            ...
            return attributes.Length == 0 ?
                   type.Name :
                   string.Format(CultureInfo.InvariantCulture, "{0} ({1})", type.Name, ((DescriptionAttribute)attributes[0]).Description);
        }
        ...
        /// </summary>
        private static int CompareEntryPoints(MethodBase? x, MethodBase? y)
        {
            return x?.DeclaringType is null && y?.DeclaringType is null
                ? 0
                : x?.DeclaringType is null
                ? -1
                : y?.DeclaringType is null ? 1 : string.Compare(x.DeclaringType.Name, y.DeclaringType.Name, StringComparison.Ordinal);
        }
