// <copyright file="StreamUtil.cs" company="MjrTom">
...
namespace MiscUtil.IO
{
    ...
    public static class StreamUtil
    {
        ...
        public static byte[] ReadFully(Stream input, byte[] buffer)
        {
            ...
            if (buffer.Length == 0)
            {
                throw new ArgumentException("Buffer is a zero-length array", nameof(buffer));
            }
        ...
        public static void Copy(Stream input, Stream output, byte[] buffer)
        {
            ...
            ArgumentOutOfRangeException.ThrowIfLessThan(buffer.Length, 1);
        ...
        public static byte[] ReadExactly(Stream input, byte[] buffer, int startIndex, int bytesToRead)
        {
            ...
            ArgumentOutOfRangeException.ThrowIfLessThan(bytesToRead, 1);
            ArgumentOutOfRangeException.ThrowIfGreaterThan(startIndex + bytesToRead, buffer.Length);
            ...
            while (index < bytesToRead)
            {
                ...
                if (read == 0)
                {
                    throw new EndOfStreamException
                        (string.Format(CultureInfo.InvariantCulture, "End of stream reached with {0} byte{1} left to read.",
                                       bytesToRead - index,
                                       bytesToRead - index == 1 ? string.Empty : "s"));
                }
